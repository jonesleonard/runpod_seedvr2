...
### Additional Security Guidelines for AWS

#### Confused Deputy Prevention for IAM Trust Policies

When creating IAM roles that are assumed by AWS services (e.g., ECS tasks, Lambda), implement confused deputy protection using `aws:SourceArn` and `aws:SourceAccount` conditions in trust policies. This prevents unauthorized external services from assuming your roles.

##### Recommended Trust Policy Condition:

```json
"Condition": {
  "ArnLike": {
    "aws:SourceArn": "arn:aws:ecs:region:account-id:*"
  },
  "StringEquals": {
...
Apply this to all service-assumed roles including ECS task roles, ECS task execution roles, and Lambda execution roles.

#### Amazon ECS and S3 Bucket Ownership Validation
